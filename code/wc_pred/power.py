...
def _build_team_matrix(data, target_col):
    ...
    print('Number of games: {0}'.format(nrows))
    ...
    for game in range(nrows):
        ...
        #print('home: {0}'.format(home['is_home'] == 1))
        if home['seasonid'] != current_season:
            # Discount older seasons.
            current_season = home['seasonid']
            current_discount *= 0.6
            print('New season {0} -- Discount: {1}'.format(current_season, current_discount))
...
def _get_power_map(competition, competition_data, col, coerce_fn):
    ...
    while True:
        if alpha < 0.1:
            print('Skipping power ranking for competition {0} column {1}'.format(
                competition, col))
            return {}
        try:
...
#            print(games)
            ...
            if not competition_power:
                alpha /= 2
                print('Reducing alpha for {0} to {1} due lack of range'.format(
                    competition, alpha))
            else:
                return competition_power
        except LinAlgError:
            alpha /= 2  
            print('Reducing alpha for {0} to {1} due to error {2}'.format(
